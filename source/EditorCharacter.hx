...
class EditorCharacter extends MusicBeatState
{
    ...
    var instructionsText:String = "Hotkeys and stuff:"
    ...
    override function create()
    {
        ...
        gridBG = FlxGridOverlay.create(20, 20);
		add(gridBG);

        var saveButton:FlxButton = new FlxButton(0, 0, "SAVE FILE", saveLevel);
        ...
        saveButton.setPosition(EXTRAS_box.x - saveButton.width - 10, EXTRAS_box.y + 10);
    ...
    function addAnimStuff():Void
    {
        ...
        animationDropdown = new FlxUIDropDownMenu(10, availableAnimations.y + availableAnimations.height, FlxUIDropDownMenu.makeStrIdLabelArray(holypiss, true), function(a:String) {
        ...
        frameRate = new FlxInputTextFix(animationIndices.width + 20, indicesTitle.y + indicesTitle.height, Std.int((animationDropdown.width / 2) - 10), camHUD);

        var postfixTitle = new FlxText(animationIndices.width + frameRate.width + 30, 80, 0, "Postfix");
        postfix = new FlxInputTextFix(animationIndices.width + frameRate.width + 30, indicesTitle.y + indicesTitle.height, Std.int((animationDropdown.width / 2)) - 3, camHUD);
        ...
        var addUpdateButton = new FlxButton(0, 0, "Add/Update", function()
        {
			...
			if (newAnim.indices != null && newAnim.indices.length > 0)
                character.animation.addByIndices(newAnim.name, newAnim.prefix, newAnim.indices, newAnim.postfix, newAnim.frameRate, newAnim.loopedAnim, newAnim.flipX, newAnim.flipY);
            else
    ...
    function addMiscStuff():Void
    {
        ...
        initialAnimationText = new FlxInputTextFix(10, initialAnimationTitle.y + initialAnimationTitle.height, Std.int(animationDropdown.width), character.initAnim, camHUD);
        initialAnimationText.callback = function(a:String, b:String) {character.initAnim = a;}

        var isDeathTick = new FlxUICheckBox(10, 50, null, null, "Death screen character?");
        isDeathTick.checked = character.isDeath;
        isDeathTick.callback = function() {character.isDeath = isDeathTick.checked;};
        ...
        facesLeftTick.callback = function() {character.facesLeft = facesLeftTick.checked;};
    ...
    function addCharacterStuff():Void
    {
        ...
        characterDropdown = new FlxUIDropDownMenu(10, characterDropdownTitle.y + characterDropdownTitle.height, FlxUIDropDownMenu.makeStrIdLabelArray(CoolUtil.coolTextFile(Paths.txt('characterList')), true));
        ...
        var loadCharJSON:FlxButton = new FlxButton(characterDropdown.x + characterDropdown.width + 10, characterDropdown.y, "Load Char JSON", function() {
            ...
            ConfirmationPrompt.confirmDisplay = "Sure";
            ConfirmationPrompt.denyDisplay = "Nah";
            ConfirmationPrompt.titleText = "Yo, wait a sec!";
            ConfirmationPrompt.descText = "Remember to save before you load a different character's JSON file! Your current changes will not be saved!";
            ConfirmationPrompt.confirmThing = function() { FlxG.switchState(new EditorCharacter(opponentTick.checked, characterDropdown.selectedLabel)); };
            ConfirmationPrompt.denyThing = function() { FlxG.mouse.visible = camHUD.visible = true; };
    ...
    /*
    function findHelperDesc(title:String):String
    {
        var toReturn:String = "";
    ...
    */
        ...
        if (character.animationsArray != null && character.animationsArray.length > 0)
        {
            for (anim in character.animationsArray)
            {
                if (anim.indices != null && anim.indices.length > 0)
                    character.animation.addByIndices(anim.name, anim.prefix, anim.indices, anim.postfix, anim.frameRate, anim.loopedAnim, anim.flipX, anim.flipY);
                else
    ...
    override function update(elapsed:Float) 
    {
        ...
        if (!isTyping)
        {
            ...
            if (FlxG.keys.justPressed.E && FlxG.camera.zoom >= 0)
                FlxG.camera.zoom += 0.25;
            if (FlxG.keys.justPressed.Q && FlxG.camera.zoom >= 0)
                FlxG.camera.zoom -= 0.25;
            ...
            if (upP || rightP || downP || leftP)
            {
                ...
                character.animationsArray[curAnim].offset = [character.animOffsets.get(character.animationsArray[curAnim].name)[0],
