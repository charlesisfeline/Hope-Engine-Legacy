...
enum abstract Action(String) to String from String
{
	...
	var UI_DOWN = "ui-down";
	var UI_UP_P = "ui-up-press";
	var UI_LEFT_P = "ui-left-press";
	var UI_RIGHT_P = "ui-right-press";
	var UI_DOWN_P = "ui-down-press";
	var UI_UP_R = "ui-up-release";
	var UI_LEFT_R = "ui-left-release";
	var UI_RIGHT_R = "ui-right-release";
	var UI_DOWN_R = "ui-down-release";
	var UI_ACCEPT = "ui-accept";
	var UI_BACK = "ui-back";
	var UI_RESET = "ui-reset";
}
#else
// this will always go unused now 
@:enum
...
class Controls extends FlxActionSet
{
	...
	var _UIaccept = new FlxActionDigital(Action.UI_ACCEPT);
	var _UIback = new FlxActionDigital(Action.UI_BACK);
	var _UIreset = new FlxActionDigital(Action.UI_RESET);
	...
	public var UI_ACCEPT(get, never):Bool;

	inline function get_UI_ACCEPT()
		return _UIaccept.check();

	public var UI_BACK(get, never):Bool;

	inline function get_UI_BACK()
		return _UIback.check();

	public var UI_RESET(get, never):Bool;

	inline function get_UI_RESET()
		return _UIreset.check();
	...
	function getActionFromControl(control:Control):FlxActionDigital
	{
		return switch (control)
		{
			...
			case UI_ACCEPT: _UIaccept;
			case UI_BACK: _UIback;
			case UI_RESET: _UIreset;
		}
	}
	...
	function forEachBound(control:Control, func:FlxActionDigital->FlxInputState->Void)
	{
		switch (control)
		{
			...
			case UI_ACCEPT:
				func(_UIaccept, JUST_PRESSED);
			case UI_BACK:
				func(_UIback, JUST_PRESSED);
			case UI_RESET:
				func(_UIreset, JUST_PRESSED);
		}
	}
	...
	public function loadKeyBinds()
	{
		...
		inline bindKeys(Control.UP, FlxG.save.data.controls[0]);
		inline bindKeys(Control.DOWN, FlxG.save.data.controls[1]);
		inline bindKeys(Control.LEFT, FlxG.save.data.controls[2]);
		inline bindKeys(Control.RIGHT, FlxG.save.data.controls[3]);
		inline bindKeys(Control.PAUSE, FlxG.save.data.controls[4]);
		inline bindKeys(Control.RESET, FlxG.save.data.controls[5]);

		inline bindKeys(Control.UI_UP, FlxG.save.data.controls[6]);
		inline bindKeys(Control.UI_DOWN, FlxG.save.data.controls[7]);
		inline bindKeys(Control.UI_LEFT, FlxG.save.data.controls[8]);
		inline bindKeys(Control.UI_RIGHT, FlxG.save.data.controls[9]);
		inline bindKeys(Control.UI_ACCEPT, FlxG.save.data.controls[10]);
		inline bindKeys(Control.UI_BACK, FlxG.save.data.controls[11]);
		inline bindKeys(Control.UI_RESET, FlxG.save.data.controls[12]);
	}
