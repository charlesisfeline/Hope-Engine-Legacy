...
class NoteSkinSelection extends MusicBeatSubstate
{
    ...
    public function new()
    {
        ...
        infoText = new FlxText(0, FlxG.height * 0.8, FlxG.width, "Skins folder is at assets/skins. See \"swag\" folder on how one works.\nPress ESCAPE to save choice, press BACKSPACE to leave without saving.", 72);
            ...
            for (noteType in 0...4)
            {
                for (noteDir in 0...4)
                {
                    ...
                    if (pissArrow.animation.curAnim.name.startsWith('hold'))
                    {
                        ...
                        pissArrow.updateHitbox();
                        pissArrow.x = skinPreview.members[skinPreview.members.length - 5].x + (skinPreview.members[skinPreview.members.length - 5].width / 2) - (pissArrow.width / 2);
                        
                        pissArrow.y = 160 * 0.7 * noteType;
                        if ((pissArrow.animation.curAnim.name.contains('end')))
                            pissArrow.y = skinPreview.members[skinPreview.members.length - 5].y + skinPreview.members[skinPreview.members.length - 5].height - 4;
                    }
                    else
                    {
    ...
    function changeItem(huh:Int = 0)
    {
        ...
        if (registeredSkins[curSelected] != "default")
        {
            if (FileSystem.exists(Sys.getCwd() + "assets/skins/" + registeredSkins[curSelected] + "/normal/note_splashes.xml"))
                splashSparrow = FlxAtlasFrames.fromSparrow(options.NoteSkinSelection.loadedSplashes.get(registeredSkins[curSelected]), File.getContent(Sys.getCwd() + "assets/skins/" + registeredSkins[curSelected] + "/normal/note_splashes.xml"));
            else
                splashSparrow = null;
        }
        else
